...
class mod_lesson_renderer extends plugin_renderer_base {
    ...
    public function display_eol_page(lesson $lesson, $data) {
        ...
        if ($data->notenoughtimespent !== false) {
            ...
            // $output .= $this->paragraph("You saw this video for {$a->timespent}, which is less than the required time. <br> Please watch it again.", 'center');
            
            $url = 'https://eccetrainings.pk/mod/lesson/view.php?id='.$this->page->cm->id;
            $output .= "Rewatch: ";
            $output .= html_writer::link($url,$lesson->name,
            array('class' => 'centerpadded lessonbutton standardbutton pr-3'));

        }
        if ($data->notenoughtimespent === false) {
            $output .= $this->paragraph("Task Completed! <br>Please proceed to the next activity.", 'center');
